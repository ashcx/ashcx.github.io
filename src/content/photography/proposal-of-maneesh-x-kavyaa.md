---
# photographyType choices: corporate-private-events | stage-work | photoshoot | wedding-rom
# hidden: true keeps this gallery out of home/photography listings unless ?hidden=1 is in the URL
# coverImageIndex: N picks the Nth image (1-based, matching image-NNN- output filenames) as the cover instead of the first
title: Maneesh & Kavyaa - The Proposal
date: 2026-09-01
photographyType: wedding-rom
client: Maneesh & Kavyaa
featuredRank: 4
categoryRank: 2
publishStatus: published
summary: "Maneesh and Kavyaa’s proposal unfolded between two sides of Singapore:
  a quiet, intimate moment in an iconic garden, followed by portraits beneath
  the city lights near Marina Bay Sands. The gallery holds the anticipation,
  joy, and sense of occasion that made the evening theirs."
cardSummary: A joyful proposal set between Singapore’s gardens and city lights.
guidedContext: "Client and couple: Maneesh and Kavyaa. This proposal session
  began with an intimate proposal at an iconic garden in Singapore, then
  continued with portraits against the Singapore nightscape around the Marina
  Bay Sands area. The gallery should feel warm, romantic, and cinematic, moving
  from the quiet anticipation of the proposal to the joy of having said yes and
  the energy of the city after dark. Focus on the couple’s connection, the
  significance of the moment, and the contrast between lush garden surroundings
  and Singapore’s illuminated skyline. Do not invent the garden’s name, proposal
  date, or any details that are not visible in the images."
coverImage: /images/galleries/proposal-of-maneesh-x-kavyaa/image-001-large.webp
---


